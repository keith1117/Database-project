...
def login():
    ...
    if role == "customer":
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT username, password, airline_name FROM Airline_Staff WHERE username=%s", (user,))
            row = cur.fetchone()
        if not row or (row["password"] not in (pwd_md5, pwd)):
            flash("Invalid credentials")
            return redirect(url_for("login"))
...
def customer_purchase():
    ...
    name_on_card = request.form.get("name_on_card", email)
    card_type = request.form.get("card_type", "Credit")
    card_number = request.form.get("card_number", "").strip()
    exp_date = request.form.get("expiration_date", "").strip()  # YYYY-MM-DD (DATE in schema)
    if not (airline and flight and dep_dt and card_number and exp_date):
        flash("Missing fields")
        return redirect(url_for("customer_search"))
    with conn.cursor() as cur:
        cur.execute(
            "SELECT 1 FROM Flight WHERE airline_name=%s AND flight_number=%s AND departure_date_time=%s",
        ...
        cur.execute(
            """
            INSERT INTO Ticket(ticket_ID, customer_email, airline_name, flight_number, departure_date_time,
                               card_type, card_number, name_on_card, expiration_date, purchase_date_time)
            VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())
            """,
            (next_id, email, airline, flight, dep_dt, card_type, card_number, name_on_card, exp_date),
        )
    conn.commit()
    flash("Ticket purchased (#%s)" % next_id)
    return redirect(url_for("customer_home"))
...
def customer_reviews():
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT flight_no:=airline_name AS airline_name, flight_number, departure_date_time, rating, comment, created_at "
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            SELECT airline_name, flight_number, departure_date_time, arrival_date_time,
                   departure_airport, arrival_airport, status
            FROM Flight
            WHERE airline_name=%s AND departure_date_time BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 DAY)
            ORDER BY departure_date_time
            """,
            (airline,),
        )
        rows = cur.fetchall()
    return render_template("staff_home.html", airline=airline, rows=rows)
...
def staff_create_flight():
    ...
    airline = session["airline"]
    if request.method == "GET":
        return render_template("staff_create_flight.html", airline=airline)
    data = {
    ...
    with conn.cursor() as cur:
        # plane ownership
        cur.execute("SELECT 1 FROM Airplane WHERE airline_name=%s AND id_number=%s", (airline, data["airplane_id_number"]))
        if not cur.fetchone():
            flash("Plane does not belong to your airline")
            return redirect(url_for("staff_create_flight"))
        cur.execute(
            """
            INSERT INTO Flight(airline_name, flight_number, departure_date_time, arrival_date_time, base_price,
                               departure_airport, arrival_airport, airplane_id_number, status)
            VALUES(%(airline_name)s, %(flight_number)s, %(departure_date_time)s, %(arrival_date_time)s, %(base_price)s,
                   %(departure_airport)s, %(arrival_airport)s, %(airplane_id_number)s, %(status)s)
            """,
            data,
        )
    conn.commit()
        ...
        with conn.cursor() as cur:
            if mode == "range":
                ...
                cur.execute(
                    ...
                    FROM Ticket t
                    WHERE t.airline_name=%s AND DATE(t.purchase_date_time) BETWEEN %s AND %s
                ...
                cur.execute(
                    ...
                    FROM Ticket t
                    WHERE t.airline_name=%s AND t.purchase_date_time >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
                ...
                cur.execute(
                    ...
                    SELECT DATE_FORMAT(t.purchase_date_time, '%Y-%m') AS ym, COUNT(*) AS tickets
                    FROM Ticket t
                    WHERE t.airline_name=%s AND t.purchase_date_time >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)
                    GROUP BY ym ORDER BY ym
                    """,
                    (airline,),
                )
